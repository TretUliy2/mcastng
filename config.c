...
int parse_servers_line(char bufr[MAXLINE])
{
	...
	for (phrase = strtok(bufr, " \t"); phrase; phrase = strtok(NULL, " \t"))
	{
		...
		case 2:
			...
			break;
		default :
			fprintf(stderr, "%s: error only two words per line permited\n error: %s",
					__FUNCTION__, bufr);
			return(0);
			break;
		}
...
int parse_src(const char *phrase) {
	...
	if (phrase != NULL )
	{
		...
		{
			if (!get_if_addr(p, (struct sockaddr_in *)&server_cfg[srv_count].mifip))
			{
	...
	else
	{
		...
		{
			fprintf(stderr, "%s: error bad ip address : %s",
					__FUNCTION__, mifglob);
			return(0);
		}
		phrase = string;
	}
